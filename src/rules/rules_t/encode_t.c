...
bool Encode_T(Metaphone3 *ctx)
{
    if (Encode_T_Initial(ctx) ||
        Encode_TCH(ctx) ||
        Encode_Silent_French_T(ctx) ||
        Encode_TUN_TUL_TUA_TUO(ctx) ||
        Encode_TUE_TEU_TEOU_TUL_TIE(ctx) ||
        Encode_TUR_TIU_Suffixes(ctx) ||
        Encode_TI(ctx) ||
        Encode_TIENT(ctx) ||
        Encode_TSCH(ctx) ||
        Encode_TZSCH(ctx) ||
        Encode_TH_Pronounced_Separately(ctx) ||
        Encode_TTH(ctx) ||
        Encode_TH(ctx))
    {
        return;
