...
void Encode_W(Metaphone3 *ctx) {
  if (Encode_Silent_W_At_Beginning(ctx) || Encode_WITZ_WICZ(ctx) ||
      Encode_WR(ctx) || Encode_Initial_W_Vowel(ctx) || Encode_WH(ctx) ||
  ...
  if (ctx->m_encodeVowels &&
      StringAt(ctx->m_inWord, ctx->m_current, 2, "WE", "") &&
