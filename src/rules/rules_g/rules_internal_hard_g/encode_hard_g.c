...
bool Internal_Hard_G(Metaphone3 *ctx)
{
  ...
  // if not "-GE" at end
  if (!(((m_current + 1) == m_last) && (CharAt(w, (m_current + 1)) == 'E')) &&
      (Internal_Hard_NG(ctx) || Internal_Hard_GEN_GIN_GET_GIT(ctx) ||
       Internal_Hard_G_Open_Syllable(ctx) || Internal_Hard_G_Other(ctx)))
  {
    return true;
  }
