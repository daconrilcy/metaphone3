...
bool Internal_Hard_G_Other(Metaphone3 *ctx) {
  ...
  if ((StringAt(w, m_current, 4, "GETH", "GEAR", "GEIS", "GIRL", "GIVI", "GIVE",
                "GIFT", "GIRD", "GIRT", "GILV", "GILD", "GELD", "") &&
       !StringAt(w, (m_current - 3), 6, "GINGIV", "")) ||
      // "gish" but not "largish"
      (StringAt(w, (m_current + 1), 3, "ISH", "") && (m_current > 0) &&
       !StringAt(w, 0, 4, "LARG", "")) ||
      (StringAt(w, (m_current - 2), 5, "MAGED", "MEGID", "") &&
       !((m_current + 2) == m_last)) ||
      StringAt(w, m_current, 3, "GEZ", "") ||
      StringAt(w, 0, 4, "WEGE", "HAGE", "") ||
      (StringAt(w, (m_current - 2), 6, "ONGEST", "UNGEST", "") &&
       ((m_current + 3) == m_last) &&
       !StringAt(w, (m_current - 3), 7, "CONGEST", "")) ||
      ...
      (StringAt(w, m_current, 4, "GILL", "") &&
       (((m_current + 3) == m_last) || ((m_current + 4) == m_last)) &&
       !StringAt(w, 0, 8, "STURGILL", ""))) {
    return true;
  }
