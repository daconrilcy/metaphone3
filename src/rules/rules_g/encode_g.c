...
void Encode_G(Metaphone3 *ctx) {
  if (Encode_Silent_G_At_Beginning(ctx) || Encode_GG(ctx) || Encode_GK(ctx) ||
      Encode_GH(ctx) || Encode_Silent_G(ctx) || Encode_GN(ctx) ||
      Encode_GL(ctx) || Encode_Initial_G_Front_Vowel(ctx) || Encode_NGER(ctx) ||
      Encode_GER(ctx) || Encode_GEL(ctx) ||
      Encode_Non_Initial_G_Front_Vowel(ctx) || Encode_GA_To_J(ctx)) {
    return;
  }
