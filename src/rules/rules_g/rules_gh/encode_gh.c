...
bool Encode_GH(Metaphone3 *ctx) {
  if (ctx->m_inWord, (ctx->m_current + 1) == 'H') {
    if (Encode_GH_After_Consonant(ctx) || Encode_Initial_GH(ctx) ||
        Encode_GH_To_J(ctx) || Encode_GH_To_H(ctx) || Encode_UGHT(ctx) ||
        Encode_GH_H_Part_Of_Other_Word(ctx) || Encode_Silent_GH(ctx) ||
        Encode_GH_To_F(ctx)) {
      return true;
    }
