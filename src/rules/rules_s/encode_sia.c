...
bool Encode_SIA(Metaphone3 *ctx)
{
    ...
    if ((StringAt(w, 0, 6, "ALESIA", "ALYSIA", "ALISIA", "STASIA", "") && (m_current == 3) &&
         !StringAt(w, 0, 9, "ANASTASIA", "")) ||
    ...
    {
        if ((IsVowel(CharAt(w, (m_current - 1))) || StringAt(w, (m_current - 1), 1, "R", ""))
            ...
            && !(StringAt(w, 0, 5, "JAMES", "NICOS", "PEGAS", "PEPYS", "") ||
                 ...
                 StringAt(w, 0, 7, "MALTHUS", "HOMOOUS", "") || StringAt(w, 0, 8, "MAGLEMOS", "HOMOIOUS", "") ||
                 StringAt(w, 0, 9, "LEVALLOIS", "TARDENOIS", "") || StringAt(w, (m_current - 4), 5, "ALGES", "")))
        {
            MetaphAdd(ctx, "J");
        }
        else
