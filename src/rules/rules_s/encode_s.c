...
void Encode_S(Metaphone3 *ctx) {
  if (Encode_SKJ(ctx) || Encode_Special_SW(ctx) || Encode_SJ(ctx) ||
      Encode_Silent_French_S_Final(ctx) ||
      Encode_Silent_French_S_Internal(ctx) || Encode_ISL(ctx) ||
      Encode_STL(ctx) || Encode_Christmas(ctx) || Encode_STHM(ctx) ||
      Encode_ISTEN(ctx) || Encode_Sugar(ctx) || Encode_SH(ctx) ||
      Encode_SCH(ctx) || Encode_SUR(ctx) || Encode_SU(ctx) ||
      Encode_SSIO(ctx) || Encode_SS(ctx) || Encode_SIA(ctx) ||
      Encode_SIO(ctx) || Encode_Anglicisations(ctx) || Encode_SC(ctx) ||
      Encode_SEA_SUI_SIER(ctx) || Encode_SEA(ctx)) {
    return;
  }
