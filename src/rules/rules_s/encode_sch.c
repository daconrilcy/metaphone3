...
bool Encode_SCH(Metaphone3 *ctx)
{
    ...
    if (StringAt(w, (m_current + 1), 2, "CH", ""))
    {
        ...
        if ((StringAt(w, (m_current + 3), 2, "OO", "ER", "EN", "UY", "ED", "EM", "IA", "IZ", "IS", "OL", "") &&
             !StringAt(w, m_current, 6, "SCHOLT", "SCHISL", "SCHERR", "")) ||
            StringAt(w, (m_current + 3), 3, "ISZ", "") ||
            (StringAt(w, (m_current - 1), 6, "ESCHAT", "ASCHIN", "ASCHAL", "ISCHAE", "ISCHIA", "") &&
             !StringAt(w, (m_current - 2), 8, "FASCHING", "")) ||
            (StringAt(w, (m_current - 1), 5, "ESCHI", "") && ((m_current + 3) == m_last)) || (CharAt(w, (m_current + 3)) == 'Y'))
        {
            // e.g. "schermerhorn", "schenker", "schistose"
            if (StringAt(w, (m_current + 3), 2, "ER", "EN", "IS", "") && (((m_current + 4) == m_last) ||
