...
Encode_Silent_K(Metaphone3 *ctx)
{
    ...
    {
        if (!(StringAt(w, (m_current + 2), 5, "ESSET", "IEVEL", "") || StringAt(w, (m_current + 2), 3, "ISH", "")))
        {
            ctx->m_current += 1;
            return true;
        }
    ...
    if ((StringAt(w, (m_current + 1), 3, "NOW", "NIT", "NOT", "NOB", "")
        ...
        StringAt(w, (m_current + 1), 4, "NOCK", "NUCK", "NIFE", "NACK", "") || StringAt(w, (m_current + 1), 5, "NIGHT", ""))
    {
...
void Encode_K(Metaphone3 *ctx)
{
    ...
    if (!Encode_Silent_K(ctx))
    {
        ...
        if ((CharAt(w, (m_current + 1)) == 'K') || (CharAt(w, (m_current + 1)) == 'Q'))
        {
            ctx->m_current += 2;
        }
        else
