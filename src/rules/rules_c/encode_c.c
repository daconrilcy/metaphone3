...
void Encode_C(Metaphone3 *ctx) {
  ...
  if (Encode_Silent_C_At_Beginning(ctx) || Encode_CA_To_S(ctx) ||
      Encode_CO_To_S(ctx) || Encode_CH(ctx) || Encode_CCIA(ctx) ||
      Encode_CC(ctx) || Encode_CK_CG_CQ(ctx) || Encode_C_Front_Vowel(ctx) ||
      Encode_Silent_C(ctx) || Encode_CZ(ctx) || Encode_CS(ctx)) {
    return;
  }

  // Par défaut, si ce n'est pas doublé par une autre consonne
  if (!(cur >= 1 ? StringAt(ctx->m_inWord, cur - 1, 1, "C", "K", "G", "Q", "")
                 : false)) {
    MetaphAdd(ctx, "K");
  }
  ...
  if (StringAt(ctx->m_inWord, cur + 1, 2, " C", " Q", " G", "")) {
    ...
    if (StringAt(ctx->m_inWord, cur + 1, 1, "C", "K", "Q", "") &&
        !StringAt(ctx->m_inWord, cur + 1, 2, "CE", "CI", "")) {
      ctx->m_current += 2;
      // Account for Ro-ckc-liffe, etc.
      if (StringAt(ctx->m_inWord, ctx->m_current, 1, "C", "K", "Q", "") &&
          !StringAt(ctx->m_inWord, ctx->m_current + 1, 2, "CE", "CI", "")) {
        ctx->m_current++;
      }
    } else {
      ctx->m_current++;
