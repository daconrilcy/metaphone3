...
void MetaphAdd(Metaphone3 *ctx, const char *in)
{
    ...
    // Clé primaire
    if (!(strcmp(in, "A") == 0 && ctx->m_primary->length > 0 && Buffer_last_char(ctx->m_primary) == 'A'))
    {
        Buffer_append(ctx->m_primary, in);
    }

    // Clé secondaire
    if (!(strcmp(in, "A") == 0 && ctx->m_secondary->length > 0 && Buffer_last_char(ctx->m_secondary) == 'A'))
    {
...
{
    // Clé primaire
    if (!(strcmp(main, "A") == 0 && ctx->m_primary->length > 0 && Buffer_last_char(ctx->m_primary) == 'A'))
    {
        Buffer_append(ctx->m_primary, main);
    }

    // Clé secondaire (uniquement si alt n'est pas vide)
    if (!(strcmp(alt, "A") == 0 && ctx->m_secondary->length > 0 && Buffer_last_char(ctx->m_secondary) == 'A'))
    {
        if (alt && alt[0] != '\0')
        {
...
/**
 * Ajoute un caractère d'encodage dans les buffers selon le mode Exact/Approx (version 4 paramètres)
 *
 * Si le flag m_encodeExact est activé, on ajoute les valeurs "exactes" (mainExact, altExact).
 * Sinon, on ajoute les valeurs "approx" (main, alt).
 *
 ...
 */
void MetaphAddExactApprox(Metaphone3 *ctx,
                          const char *mainExact, const char *altExact,
                          const char *main, const char *alt)
{
...
/**
 * Ajoute un caractère d'encodage dans les buffers selon le mode Exact/Approx (version 2 paramètres)
 *
 * Si le flag m_encodeExact est activé, on ajoute la valeur "exacte" (mainExact).
 * Sinon, on ajoute la valeur "approx" (main).
 *
 * @param ctx       Contexte Metaphone3
 * @param mainExact Chaîne à ajouter à la clé primaire et secondaire en mode exact
 * @param main      Chaîne à ajouter à la clé primaire et secondaire en mode approx
 */
